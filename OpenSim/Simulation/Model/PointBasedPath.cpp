...
public:
                                     ...
                                     const OpenSim::AbstractPathPoint* ptr)
    {
        ...
        OPENSIM_THROW(OpenSim::Exception, "unable to create a PathElementLookup for the given pointer: are you sure it is part of the PointBasedPath?");
    }
...
static void PopulatePathPointersCache(
    ...
    const OpenSim::PathWrapSet& pws,
    const std::vector<OpenSim::PointBasedPath::PathElementLookup>& lookups,
...
static void PopulatePathElementLookup(
    ...
    const OpenSim::PointBasedPath& geomPath,
    std::vector<OpenSim::PointBasedPath::PathElementLookup>& out)
{
    out.clear();
    out.reserve(ptrs.getSize());
    for (int i = 0, len = ptrs.getSize(); i < len; ++i) {
        out.push_back(OpenSim::PointBasedPath::PathElementLookup::fromPtr(pps, pws, geomPath, ptrs[i]));
    }
}
...
*
*/
void PointBasedPath::extendFinalizeFromProperties()
{
...
void PointBasedPath::extendConnectToModel(Model& aModel)
{
 ...
 void PointBasedPath::extendAddToSystem(SimTK::MultibodySystem& system) const
{
 ...
 void PointBasedPath::extendInitStateFromProperties(SimTK::State& s) const
{
 ...
 */
void PointBasedPath::constructProperties()
{
 ...
 */
void PointBasedPath::namePathPoints(int aStartingIndex)
{
    char indx[5];
 ...
 * 
 */
const OpenSim::Array <AbstractPathPoint*> & PointBasedPath::
...
void PointBasedPath::addInEquivalentForces(const SimTK::State& s,
 ...
 * 
 */
void PointBasedPath::updateGeometry(const SimTK::State& s) const
{
...
void PointBasedPath::setLength( const SimTK::State& s, double length ) const
{
    setCacheVariableValue(s, _lengthCV, length);
}

void PointBasedPath::setColor(const SimTK::State& s, const SimTK::Vec3& color) const
{
    setCacheVariableValue(s, _colorCV, color);
}

Vec3 PointBasedPath::getColor(const SimTK::State& s) const
{
...
void PointBasedPath::setLengtheningSpeed( const SimTK::State& s, double speed ) const
{
 ...
 */
AbstractPathPoint* PointBasedPath::
 ...
 */
bool PointBasedPath::canDeletePathPoint( int aIndex)
{
 ...
 */
bool PointBasedPath::deletePathPoint(const SimTK::State& s, int aIndex)
{
 ...
 */
void PointBasedPath::addPathWrap(WrapObject& aWrapObject)
{
 ...
 */
void PointBasedPath::moveUpPathWrap(const SimTK::State& s, int aIndex)
{
    if (aIndex > 0) {
 ...
 */
void PointBasedPath::moveDownPathWrap(const SimTK::State& s, int aIndex)
{
 ...
 */
void PointBasedPath::deletePathWrap(const SimTK::State& s, int aIndex)
{
    upd_PathWrapSet().remove(aIndex);
 ...
 */
void PointBasedPath::computePath(const SimTK::State& s) const
{
 ...
 */
void PointBasedPath::computeLengtheningSpeed(const SimTK::State& s) const
{
...
void PointBasedPath::updateFromXMLNode(SimTK::Xml::Element& aNode, int versionNumber)
{
